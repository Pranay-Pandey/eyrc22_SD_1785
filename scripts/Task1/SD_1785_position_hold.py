...
class Edrone():
	...
	def __init__(self):
		...
		self.drone_command = [self.cmd.rcRoll, self.cmd.rcPitch, self.cmd.rcThrottle]
	...
	#	8. Add error_sum
		...
		if (self.timechange>self.sample_time):
			if (self.last_time!=0):			
				...
				self.cmd.rcRoll=1500-int((self.Kp[0]*self.error[0])+(self.Kd[0]*self.derr[0])-(self.errsum[0]*self.Ki[0]))
				self.cmd.rcPitch=1500+int((self.Kp[1]*self.error[1])+(self.Kd[1]*self.derr[1])-(self.errsum[1]*self.Ki[1]))
				self.cmd.rcThrottle=1500+int((self.Kp[2]*self.error[2])+(self.Kd[2]*self.derr[2])-(self.errsum[2]*self.Ki[2]))
				...
				self.cmd.rcThrottle = self.cmd.rcThrottle if self.error[2] >= 0.1 else 1499
